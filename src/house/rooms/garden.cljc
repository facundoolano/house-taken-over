...
(defn crow-post
  ...
  (let [door (utils/find-first gs "door")
    ...
    (utils/say "Easy job for the crowbar. The door was now open.")
    (-> gs
...
(def crowbar (item/make "crowbar" "satisfy all your breaking-into-places needs."
                        :take true
                        :unlock "Unlock with a crowbar? That's one way to put it, I guess." ; FIXME this is working backwards
                        :use-with {:pre `crow-pre :post `crow-post}))
